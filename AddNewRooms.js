...
async function addNewRoomInFirebase() {
  ...
  try {
    const response = await fetch('https://ashraf-baec8-default-rtdb.firebaseio.com/Rooms.json');
    ...
    await fetch(`https://ashraf-baec8-default-rtdb.firebaseio.com/Rooms/${index}.json`, {
      method: 'PUT',
      headers: {
...
function DisplayData() {
  ...
  request.open('GET', 'https://ashraf-baec8-default-rtdb.firebaseio.com/Rooms.json');
...
function deleteRoom(roomKey) {
  // إنشاء طلب HTTP
  const request = new XMLHttpRequest();
  request.open('DELETE', `https://ashraf-baec8-default-rtdb.firebaseio.com/Rooms/${roomKey}.json`);
